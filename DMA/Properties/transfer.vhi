...
for timepoints:
  ...
  t_write_req=t_read_ack+1,
  t_write_ack = t_write_req + 0..max_wait waits_for complete rdy_i = '1';
...
assume:
  ...
  at t: config_reg(command_addr_g)(0)='1';
  
prove:

  -- conceptual state
  at t_write_ack+1: idle_state;

  -- outputs
  during [t+1, t_read_ack]:          ram_read(src_addr_f);
  during [t_write_req, t_write_ack]: ram_write(tgt_addr_f, data_f);
  at t_write_ack+1:                  ram_idle;

  during [t+1, t_write_ack+1]: rdy_o = '0';
  during [t+1, t_write_ack+1]: if continue_acc_f then
                                 bus_request_o = '1'
                               else
                                 bus_request_o = '0'
                               end if;
  ...
  -- visible registers
  at t_write_ack+1: if continue_acc_f then
  ...
  at t_write_ack+1: config_reg(parity_addr_g) = new_parity_value;

right_hook: t_write_ack+1;
end property;
